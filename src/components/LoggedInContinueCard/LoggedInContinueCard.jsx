...
const LoggedInContinueCard = ({ redirectTo = "/", isSw = false }) => {
  ...
  return (
    <Wrapper>
      ...
      <Title>{isSw ? "Umeshaingia" : "You're logged in"}</Title>
      {displayName && <Sub>{displayName}</Sub>}
      <ContinueLink to={redirectTo}>
        {isSw ? "Endelea na Uthibitishaji" : "Continue to Verification"}
        <FaArrowRight />
      </ContinueLink>
      <LogoutBtn type="button" onClick={handleLogout}>
        {isSw ? "Si wewe? Ondoka" : "Not you? Log out"}
      </LogoutBtn>
    </Wrapper>
  );
};
